...
public class EaseChatMessageList extends RelativeLayout{
     ...
     */
    public void init(String toChatUsername, String toChatUserHead,String myHead,int chatType, EaseCustomChatRowProvider customChatRowProvider) {
        this.chatType = chatType;
        this.toChatUsername = toChatUsername;
        
        conversation = EMClient.getInstance().chatManager().getConversation(toChatUsername, EaseCommonUtils.getConversationType(chatType), true);
        Log.e(TAG, "init: toChatUserHead = "+toChatUserHead +"----myHead ="+myHead );
        List<EMMessage> allMessages = conversation.getAllMessages();
        for (int i = 0; i < allMessages.size(); i++) {
            if(allMessages.get(i).direct() != EMMessage.Direct.SEND){
                allMessages.get(i).setAttribute(EaseConstant.EXTRA_USER_OTHER_HEAD, toChatUserHead);
            }else {
                allMessages.get(i).setAttribute(EaseConstant.EXTRA_USER_MY_HEAD, myHead);
            }
        }
